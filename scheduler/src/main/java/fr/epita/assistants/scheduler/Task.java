...
public interface Task<RETURN_TYPE> {
    ...
    /**
     * Chain a new task ting an early get on the CompletableFuture will interrupt the execution flow of your program and
will not pass the tests. A task should block the execution flow only when needed, meaning that it
should be done only in the executeo be executed after the current one, taking the output of the current one as its input.
     *
